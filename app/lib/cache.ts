...
/**
 * Async cache revalidation for upvote changes (optimized)
 * Only revalidates essential paths to improve performance
 */
export function revalidateFeatureRequestsAsync(slug: string, userId?: string) {
 // Use setImmediate to make this truly async and non-blocking
 setImmediate(() => {
  try {
   // Only revalidate the most critical paths for upvotes
   revalidatePath(`/b/${slug}`);
   revalidateTag(`${CACHE_TAGS.FEATURE_REQUESTS}-${slug}`);

   // Only revalidate dashboard if userId provided
   if (userId) {
    revalidateTag(`${CACHE_TAGS.DASHBOARD_STATS}-${userId}`);
   }
  } catch (error) {
   // Log errors but don't let them affect the main request
 ...
 */
export function revalidateComments(
